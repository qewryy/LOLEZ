			...
			for (String fm : matchIdList) {
				...
				if (!(fm.equals(matchId))) {
					...
					if (match.getMetadata() != null && match.getInfo() != null) {
						...
						String combined = String.join(",", match.getMetadata().getParticipants());
						match.getMetadata().setParticipants_String(combined);
						int Perki = 0;
						int MTir = mdao.insertmetadata(match.getMetadata());

						// Metadata DB Insert 처리 여부
						if (MTir == 1) {
							System.out.println("Metadata DB Insert 성공");
							System.out.println("\nDB에 Info Insert 실행");
							int IFir = mdao.insertinfo(match.getInfo());
							...
							if (IFir == 1) {
								...
								for (int i = 0; i < match.getInfo().getTeams().size(); i++) {
									match.getInfo().getTeams().get(i).setGameId(match.getInfo().getGameId());

									System.out.println("\nDB에 TeamDto Insert 실행");
									int TMir = mdao.insertteam(match.getInfo().getTeams().get(i));
									...
									if (TMir == 0) {
										...
										System.out.println("\nDB에 BanDto Insert 실행");
											...
											for (int a = 0; a < match.getInfo().getTeams().get(i).getBans().size(); a++) {
												...
												bl.setGameId(match.getInfo().getGameId());
								...
								for (int i = 0; i < match.getInfo().getParticipants().size(); i++) {
									ParticipantDto pl = match.getInfo().getParticipants().get(i);
									pl.setPuuid(list.get(i));
									pl.setGameid(match.getInfo().getGameId());
									...
									if (PTir == 0) {
										...
										psl.setGameId(match.getInfo().getGameId());
										...
										if (PSTir == 1) {
											...
											for (int a = 0; a < pl.getPerks().getStyles().size(); a++) {
												...
												pyl.setGameId(match.getInfo().getGameId());
