...
public class EmailController {
	...
	public ModelAndView emailprove(String email) throws Exception {
		...
		if (ck == null) {
			byte[] textBytes = email.getBytes(StandardCharsets.UTF_8);
			String encodedText = Base64.getEncoder().encodeToString(textBytes);
			token = UUID.randomUUID().toString()+"_"+encodedText;
		} else if (ck.getPstatus() == 0) {
			token = ck.getPtoken();
		} else if (ck.getPstatus() == 1) {
			System.out.println("이메일 인증 완료 대상입니다.");

			mav.setViewName("redirect:/JoinForm?email="+email);
			return mav;
		}
		...
		String contect = "<!DOCTYPE html>" + "<html lang=\"en\">" + "<head>" + "    <meta charset=\"UTF-8\">"
				...
				+ "                        회원님의 이메일 <a href=\"mailto:\""+ email +" target=\"_blank\">"+email+"</a>로 LOL.EZ의 계정을 생성 중 입니다."
				...
				+ "                </tr>" 
				+ "                <tr>"
				+ "                    <td height=\"21\" style=\"font-size:18px\">" 
				+ "                        인증 링크"
				+ "                    </td>" 
				+ "                </tr>" 
				+ "                <tr>"
		...
		if (ck == null) {
			...
			System.out.println("이메일 인증 토큰 발급 기록이 있습니다.");
	...
	public ModelAndView verify(String token) throws Exception {
		...
		if(Ptoken == null){
			...
			byte[] decodedBytes = Base64.getDecoder().decode(tokenEmail[1]);
