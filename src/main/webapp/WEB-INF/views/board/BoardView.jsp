...
<body>
						...
						<div class="p-5">
							<form class="user" action="" method="post">
								...
								<div class="form-group row">
                                    ...
                                    <c:if test="${board.bwriter == sessionScope.loginNickname }">
	                            ...
	                            <div class="card shadow">
	                                <div class="card-body p-3">
	                                    <div class="row no-gutters align-items-center text-xs font-weight-bold">
		                                	<div class="col">
					                            <span class="text-uppercase pl-2">${reply.rdate }</span>
				                            </div>
				                            <div class="col-auto">
				                            	<c:choose>
				                            		<c:when test="${reply.rlikeCheck == null }">
						                            	<button class=" text-xs btn btn-sm border-primary btn-user text-primary" onclick="replyLike('${reply.rno}',this)">
						                            	추천 <span> ${reply.rlikecount } </span>
						                            	</button>
				                            		</c:when>
				                            		<c:otherwise>
						                            	<button class=" text-xs btn btn-sm border-primary btn-user text-white bg-primary" onclick="replyLike('${reply.rno}',this)">
						                            	추천 <span> ${reply.rlikecount } </span>
						                            	</button>				                            		
				                            		</c:otherwise>
				                            	</c:choose>
	                                    ...
	                                    <div class="row no-gutters align-items-center">
		                                	<div class="col">
	                                            ...
	                                            class="retext mb-2 border-0 font-weight-bold text-gray-800 w-100">${reply.rcontent }</textarea>
				                        	</div>
				                            <div class="col-auto">
				                            	<c:if test="${reply.rname == sessionScope.loginId}">
				                            	<button class="btn btn-sm btn-danger btn-user" onclick="replyDelete_ajax('${reply.rno}')">삭제</button>
				                            	</c:if>
				                            </div>	                                    
	                                    </div>	                                    
	                                </div>
                    ...
                    <div class="col-lg-7 ml-auto mr-auto" style="background-color: white;" >
                        <div class="pt-1 px-5 pb-1">
                            <form class="user" onsubmit="return replyWrite(this)">
                            	<input type="hidden" name="rbno"  value="${board.bno }">
                            	<input type="hidden" name="rname"  value="${sessionScope.loginNickname}">
                                <div class="form-group">
		...
		$.ajax( { 
			...
			success : function(reList){
				...
				for(var i = 0; i < reList.length; i++){
					console.log(reList[i].rname + " : " + reList[i].rcontent);
					...
					output += '<span class="text-uppercase pl-2 text-xs ">'+reList[i].rdate+'</span>';
					output += '</div>';
					output +='<div class="col-auto align-items-center">';
					
					if(reList[i].rlikeCheck == null){
						output += '<button class=" text-xs btn btn-sm border-primary btn-user text-primary" onclick="replyLike('+ "'"+reList[i].rno+"'" +',this)">';
					} else {
						output += '<button class=" text-xs btn btn-sm border-primary btn-user text-white bg-primary" onclick="replyLike('+ "'"+reList[i].rno+"'" +',this)">';
					}
					...
					output += '<div class="col">';
					output += '<textarea readonly="readonly"class="retext mb-2 border-0 font-weight-bold text-gray-800 w-100">'+reList[i].rcontent+'</textarea>';
					output += '</div>';
					output += '<div class="col-auto">';
					if(reList[i].rname == loginId){
						output += '<button class="btn btn-sm btn-danger btn-user" onclick="replyDelete_ajax(\''+reList[i].rno+'\')" >삭제</button>';
	...
	function replyLike(rno,likeBtn){
		if(loginId.length > 0 ){
			$.ajax( { 
				type : "get",
				url : "${pageContext.request.contextPath }/replyLike",
				data : { "rlnum" : rno, "rlmid" : loginId },
				dataType:"json",
				async:false,
				success : function(result){
					if(result.likeResult == '1'){
