											...
											<div class="col-sm-12 mb-3 mb-sm-0 modifyCol">
												<div class="col-sm-6">
													<a href="${pageContext.request.contextPath }/boardList"
														class="btn btn-primary btn-user btn-block">글목록 </a>
												</div>
										</c:otherwise>
									</c:choose>
								</div>
								<c:choose>
									<c:when test="${board.bwriter == sessionScope.loginNickname }">
										<div class="col-sm-3 mb-3 mb-sm-0 modifyCol d-none">
											<button type="submit"
												class="btn btn-success btn-user btn-block ">수정</button>
										</div>
										<div class="col-sm-3 modifyCol d-none">
											<button type="button" onclick="toggleModForm('close')"
												class="btn btn-danger btn-user btn-block ">취소</button>
										</div>
										<div class="col-sm-6">
											<a href="${pageContext.request.contextPath }/boardList"
												class="btn btn-primary btn-user btn-block"> 글목록 </a>
							...
							<c:forEach items="${replyList }" var="reply">
								<div class="card shadow">
									<div class="card-body p-3">
										<div
											class="row no-gutters align-items-center text-xs font-weight-bold">
											<div class="col">
												<span class="text-primary">${reply.rname}</span> <span
													class="text-uppercase pl-2">${reply.rdate }</span>
											</div>
											<div class="col-auto">
												<c:choose>
													<c:when test="${reply.rlikeCheck == null }">
														<button
															...
															onclick="replyLike('${reply.rno}','${reply.rbno}',this)">
															추천 <span> ${reply.rlikecount } </span>
														</button>
													</c:when>
													<c:otherwise>
														<button
															class=" text-xs btn btn-sm border-primary btn-user text-white bg-primary"
															onclick="replyLike('${reply.rno}','${reply.rbno}',this)">
															추천 <span> ${reply.rlikecount } </span>
														</button>
													</c:otherwise>
										...
										<div class="row no-gutters align-items-center">
											...
											<div class="col-auto">
												<c:if test="${reply.rname == sessionScope.loginNickname}">
													<button class="btn btn-sm btn-danger btn-user"
														onclick="replyDelete_ajax('${reply.rno}','${reply.rbno}')">삭제</button>
												</c:if>
											</div>
										</div>
									</div>
						...
						<div class="pt-1 px-5 pb-1">
							<form class="user" onsubmit="return replyWrite(this)">
								...
								<div class="form-group">
									...
									<textarea name="rcontent" id="inputrcontent"
